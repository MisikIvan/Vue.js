...
export default {
    ...
    methods: {
        // Check if the user is logged in and retrieve the auth token
        checkAuthentication() {
            const auth = getAuth();
            onAuthStateChanged(auth, async (user) => {
                if (user) {
                    this.isAuthenticated = true;
                    this.authToken = await user.getIdToken();  // Get the auth token for requests
                    this.fetchItems(); // Fetch items after obtaining token
                } else {
        ...
        // GET /api/items - Fetch items from Firebase
        async fetchItems() {
            try {

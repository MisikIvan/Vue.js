<template>
    <div class="space-y-2">
        ...
        <input type="number" v-model="numericAnswer" :min="question.min" :max="question.max"
...
const { value: numericAnswer, errorMessage: error } = useField(
    ...
    yup
        ...
        .min(props.question.min || Number.NEGATIVE_INFINITY, `Value must be at least ${props.question.min}`)
        .max(props.question.max || Number.POSITIVE_INFINITY, `Value must be at most ${props.question.max}`)
);
